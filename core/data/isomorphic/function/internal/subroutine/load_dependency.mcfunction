...
### Usage:
# run isomorphic:internal/load_dependency {path:<string>,dependency:<string>}
### Assumes:
# local.log_msg.datapack is set correctly.
###

# Demonstrates what we are attempting to accomplish.
$data modify storage isomorphic:temp local.log_msg.message set value [{text:"Attempting to load the dependency $(dependency) before $(path)."}]
...
$execute store success storage isomorphic:temp local.success_code int 1 run datapack enable "$(dependency)" before "$(path)"
